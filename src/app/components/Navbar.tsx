...
const Navbar = () => {
  return (
    <nav className='fixed mx-auto top-0 left-0 right-0 z-10 bg-[#121212] bg-opacity-100'>
      <div className='flex container lg:py-4 flex-wrap items-center justify-between mx-auto md:px-14 px-4 py-2'>
        <Link
          href={'/'}
        ...
        <div className='menu md:w-auto' id='navbar'>
          <ul className='flex gap-4 md:space-x-8 mt-0'>
            {navLinks.map((link, index) => (
              <li key={index}>
                <Link
                  href={link.path}
                  className='block py-2 text-[#ADB7BE] sm:text-xl rounded md:p-0 hover:text-white'
